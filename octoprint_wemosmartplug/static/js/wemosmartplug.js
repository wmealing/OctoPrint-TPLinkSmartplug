/*
 * View model for OctoPrint-WemoSmartplug
 *
 * Author: jneilliii
 * License: AGPLv3
 */
$(function() {
    function wemosmartplugViewModel(parameters) {
        var self = this;
		...
		self.onBeforeBinding = function() {		
			self.arrSmartplugs(self.settings.settings.plugins.wemosmartplug.arrSmartplugs());
        }
		...
		self.addPlug = function() {
			...
			self.settings.settings.plugins.wemosmartplug.arrSmartplugs.push(self.selectedPlug());
			$("#WemoPlugEditor").modal("show");
		}
		
		self.removePlug = function(row) {
			self.settings.settings.plugins.wemosmartplug.arrSmartplugs.remove(row);
		}
		
		self.onDataUpdaterPluginMessage = function(plugin, data) {
            if (plugin != "wemosmartplug") {
                return;
            }

		    console.log(data)
			plug = ko.utils.arrayFirst(self.settings.settings.plugins.wemosmartplug.arrSmartplugs(),function(item){
			...
			if (plug.currentState != data.currentState) {
				...
				switch(data.currentState) {
					...
					default:
						new PNotify({
							title: 'Wemo Smartplug Error',
							text: 'Status ' + plug.currentState() + ' for ' + plug.ip() + '. Double check IP Address\\Hostname in WemoSmartplug Settings.',
							type: 'error',
							hide: true
							});
		...
		self.sendTurnOn = function(data) {
            $.ajax({
                url: API_BASEURL + "plugin/wemosmartplug",
                type: "POST",
                dataType: "json",
                data: JSON.stringify({
                    command: "turnOn",
    	...
    	self.turnOff = function(data) {
			if((data.displayWarning() || (self.isPrinting() && data.warnPrinting())) && !$("#WemoSmartPlugWarning").is(':visible')){
				self.selectedPlug(data);
				$("#WemoSmartPlugWarning").modal("show");
			} else {
				$("#WemoSmartPlugWarning").modal("hide");
				if(data.sysCmdOff()){
		...
		self.checkStatuses = function() {
			ko.utils.arrayForEach(self.settings.settings.plugins.wemosmartplug.arrSmartplugs(),function(item){
				if(item.ip() !== "") {
					console.log("checking " + item.ip())
					self.checkStatus(item.ip());
				}
			});
			if (self.settings.settings.plugins.wemosmartplug.pollingEnabled()) {
				setTimeout(function() {self.checkStatuses();}, (parseInt(self.settings.settings.plugins.wemosmartplug.pollingInterval(),10) * 60000));
			};
        };
    }
    ...
    OCTOPRINT_VIEWMODELS.push([
        wemosmartplugViewModel,

        // e.g. loginStateViewModel, settingsViewModel, ...
        ["settingsViewModel","loginStateViewModel"],

        // "#navbar_plugin_wemosmartplug","#settings_plugin_wemosmartplug"
        ["#navbar_plugin_wemosmartplug","#settings_plugin_wemosmartplug"]
    ]);
});
